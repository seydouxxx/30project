...
//        self.tableView.delegate = self
        ...
        self.tableView.register(nibName, forCellReuseIdentifier: "CardListTableViewCell")
...
//        }
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let card = creditCardList[indexPath.row]
        
        cell.rankLabel.text = "\(card.rank)위"
        cell.promotionLabel.text = "\(card.promotionDetail.amount)만원 증정"
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        let card = creditCardList[indexPath.row]
        detailViewController.promotionDetail = card.promotionDetail
        self.show(detailViewController, sender: nil)
        
        //실시간 데이터베이스 쓰기
        // Option 1
//        ref.child("Item\(card.id)/isSelected").setValue(true)
        
        // Option 2
//        ref.queryOrdered(byChild: "id").queryEqual(toValue: card.id).observe(.value) {[weak self] snapshot in
//            guard let self = self,
...
//        }
        ...
        //Firestore 쓰기
        //Option 1
//        db.collection("creditCardList").document("card\(card.id)").updateData(["isSelected": true])
        
        //Option 2
        db.collection("creditCardList").whereField("id", isEqualTo: card.id).getDocuments { snapshot, error in
...
//            db.collection("creditCardList").document("card\(cardID)").delete()
            ...
            db.collection("creditCardList").whereField("id", isEqualTo: cardID).getDocuments { snapshot, _ in
                guard let snapshot = snapshot?.documents.first else {
                    print("ERROR")
                    return
                }
